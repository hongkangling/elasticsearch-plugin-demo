...
public final class ICQueryParser {

	// 查询关键字解析缓存线程本地变量
	private static ThreadLocal<Map<String, TokenBranch>> keywordCacheThreadLocal = new ThreadLocal<Map<String, TokenBranch>>();
	...
	public static void setMaxWordLength(boolean isMaxWordLength) {
		ICQueryParser.isMaxWordLength = isMaxWordLength;
	}
	...
	private static Query optimizeQueries(List<Query> queries) {
		...
		if (queries.size() == 0) {
			...
			BooleanQuery.Builder mustQueries = new BooleanQuery.Builder();
			for (Query q : queries) {
				mustQueries.add(q, Occur.MUST);
			}
			return mustQueries.build();
		}
	}
	...
	private static Query _parse(String field, String query) throws IOException {
		...
		if (root != null) {
			...
			ICSegmentation ikSeg = new ICSegmentation(isMaxWordLength);
			ikSeg.reset(input);
	...
	public static Query parse(String field, String query) throws IOException {
		...
		if (qParts.length > 1) {
			...
			for (String q : qParts) {
				...
				if (partQuery != null
						&& (!(partQuery instanceof BooleanQuery) || ((BooleanQuery) partQuery).clauses().size() > 0)) {
	 ...
	 * @throws IOException
	 */
	public static Query parseMultiField(String[] fields, String query, Occur[] flags) throws IOException {
		if (fields == null) {
			throw new IllegalArgumentException("parameter \"fields\" is null");
		}
		if (flags == null) {
	...
	private static class TokenBranch {
		...
		boolean accept(Lexeme _lexeme) {
			...
			/*
			 * 检查新的lexeme 对当前的branch 的可接受类型 acceptType : REFUSED 不能接受 acceptType
			 * : ACCEPTED 接受 acceptType : TONEXT 由相邻分支接受
			 */
		...
		List<Query> toQueries(String fieldName) {
			...
			if (acceptedBranchs != null && acceptedBranchs.size() > 0) {
				if (acceptedBranchs.size() == 1) {
					...
					BooleanQuery bquery = orQuery.build();
	...
	/**
	 * 查询表达式解析 alpha版本 自定义lucene查询表达式 表达式例子 ： (id='1231231' && title:'monkey')
	 * || (content:'你好吗' || ulr='www.ik.com') - name:'helloword' May 20, 2010
	 */
			...
			for (int i = 0; i < this.elements.size(); i++) {
				...
				if ('F' == e.type) {
					...
					if ('=' == e2.type) {
						...
						try {
							Query tQuery = ICQueryParser.parse(e.toString(), e3.toString());
							this.querys.push(tQuery);
		...
		private Query toQuery(Element op) {
			...
			if ('&' == op.type) {
				if (q1 instanceof TermQuery) {
					...
					if (clauses.get(0).getOccur() == Occur.MUST) {
				...
				if (q1 instanceof TermQuery) {
					...
					List<BooleanClause> clauses = ((BooleanQuery) q1).clauses();
					if (clauses.get(0).getOccur() == Occur.SHOULD) {
						for (BooleanClause c : clauses) {
							resultQuery.add(c);
						}
					} else {
